...
class ProviderProfileFragmentState extends State<ProviderProfileFragment> {
  ...
  Future<void> init() async {
    ...
    // Sync selectedLanguage with the exact instance from languageList
    final availableLanguages = languageList();
    final currentLanguageCode = appStore.selectedLanguage.languageCode.validate();
    final matchingLanguage = availableLanguages.firstWhere(
      (lang) => lang.languageCode.validate() == currentLanguageCode,
      orElse: () => availableLanguages.first,
    );
    if (matchingLanguage.languageCode != appStore.selectedLanguage.languageCode) {
      appStore.setSelectedLanguage(matchingLanguage);
    }
  }
      ...
      builder: (_) {
        return AnimatedScrollView(
          ...
          children: [
            ...
            if (appStore.earningTypeSubscription && appStore.isPlanSubscribe && appStore.providerCurrentSubscription?.status.validate() == SUBSCRIPTION_STATUS_ACTIVE)
              Column(
                children: [
                  16.height,
                  Container(
            ...
            SettingSection(
              ...
              items: [
                ...
                if (rolesAndPermissionStore.shop)...[
                  SettingItemWidget(
                    decoration: BoxDecoration(color: context.cardColor),
                    leading: Image.asset(
                      ic_shop,
            ...
            SettingSection(
              ...
              items: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Image.asset(language, height: 16, width: 16, color: appStore.isDarkMode ? white : appTextSecondaryColor),
                      16.width,
                      Text(languages.language, style: boldTextStyle(size: 12)).expand(),
                      Observer(
                        builder: (_) {
                          // Read observable values to ensure MobX tracks them
                          final selectedLanguageCode = appStore.selectedLanguageCode;
                          final selectedLanguageModel = appStore.selectedLanguage;

                          final availableLanguages = languageList();
                          // Use selectedLanguageCode if available, otherwise fallback to selectedLanguage.languageCode
                          final currentLanguageCode = selectedLanguageCode.validate().isNotEmpty ? selectedLanguageCode.validate() : selectedLanguageModel.languageCode.validate();

                          // Find the matching language from available languages by language code
                          final selectedLanguageIndex = availableLanguages.indexWhere(
                            (lang) => lang.languageCode.validate() == currentLanguageCode,
                          );
                          final selectedLanguage = selectedLanguageIndex >= 0 ? availableLanguages[selectedLanguageIndex] : availableLanguages.first;
                          return Container(
                            height: 30,
                            alignment: Alignment.centerRight,
                            
                            key: ValueKey('language_dropdown_$currentLanguageCode'),
                            decoration: boxDecorationDefault(
                              borderRadius: radius(defaultRadius),
                              color: context.scaffoldBackgroundColor,
                              border: Border.all(color: appStore.isDarkMode ? Colors.grey.withValues(alpha: 0.3) : Colors.grey.withValues(alpha: 0.5), width: 1.0),
                            ),
                            padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                            child: DropdownButton<LanguageDataModel>(
                              key: ValueKey('dropdown_$currentLanguageCode'),
                              value: selectedLanguage,
                              isExpanded: false,
                              underline: const SizedBox(),
                              icon: Icon(Icons.arrow_drop_down, color: appStore.isDarkMode ? white : gray.withValues(alpha: 0.8), size: 18),
                              dropdownColor: context.cardColor,
                              style: primaryTextStyle(size: 12),
                              selectedItemBuilder: (BuildContext context) {
                                // Return widgets in the exact same order as items
                                return availableLanguages.map((LanguageDataModel language) {
                                  return Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      if (language.flag.validate().isNotEmpty)
                                        Image.asset(
                                      ...
                                      if (language.flag.validate().isNotEmpty) 6.width,
                                      Flexible(
                                        child: Text(
                                          language.name.validate(),
                                          style: primaryTextStyle(size: 12),
                                          maxLines: 1,
                              ...
                              items: availableLanguages.asMap().entries.map((entry) {
                                ...
                                return DropdownMenuItem<LanguageDataModel>(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          if (language.flag.validate().isNotEmpty) Image.asset(language.flag.validate(), width: 20, height: 20, fit: BoxFit.cover),
                                          if (language.flag.validate().isNotEmpty) 8.width,
                                          Text(language.name.validate(), style: primaryTextStyle(size: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
                                        ],
                                      ),
                                      if (!isLast) Divider(height: 1, thickness: 1, color: context.dividerColor),
                                    ],
                                  ),
                                );
                              }).toList(),
                              onChanged: (LanguageDataModel? newLanguage) async {
                                if (newLanguage != null) {
                                  final newLanguageCode = newLanguage.languageCode.validate();
                                  final currentCode = appStore.selectedLanguageCode.validate().isNotEmpty ? appStore.selectedLanguageCode.validate() : appStore.selectedLanguage.languageCode.validate();

                                  if (newLanguageCode != currentCode) {
                                    await appStore.setLanguage(newLanguageCode);
                                    // Update selectedLanguage to the exact instance from the list
                                    appStore.setSelectedLanguage(newLanguage);
                                    setState(() {});
                                  }
                                }
                              },
