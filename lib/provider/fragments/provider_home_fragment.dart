...
class _ProviderHomeFragmentState extends State<ProviderHomeFragment> {
  ...
  Widget planBanner(DashboardResponse data) {
    if(data.downGradeBanner != null) {
      return subSubscriptionPlanWidget(
        planBgColor: appStore.isDarkMode ? context.cardColor : Colors.red.shade50,
        planTitle: data.downGradeBanner!.title.validate(),
        planSubtitle: data.downGradeBanner!.description.validate(),
      );
    }
    if (data.upGradeBanner != null) {
      return subSubscriptionPlanWidget(
        planBgColor: appStore.isDarkMode ? context.cardColor : Colors.orange.shade50,
        planSubtitle: data.upGradeBanner!.description.validate(),
        icon: Icons.info,
      );
    }
    if (data.subscription?.status == 'cancelled') {
      return subSubscriptionPlanWidget(
        planBgColor: appStore.isDarkMode ? context.cardColor : Colors.orange.shade50,
        planTitle: languages.lblPlanCancelled,
        planSubtitle: languages.lblPlanCancelledSubtitle,
        planButtonTxt: languages.lblUpgrade,
