...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  TextEditingController locationPickCont = TextEditingController();
  ...
  /// Provider address (from Google Places)
  double? providerLat;
  double? providerLng;
              ...
              child: Center(
                child: ValueListenableBuilder(
                  ...
                  builder: (context, value, child) {
                    final String code = selectedCountry.phoneCode;
                    ...
                    return Row(
                      children: [
                        Text(
                          '${selectedCountry.flagEmoji} $formattedCode',
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                onChanged: (c) {
                  ...
                  if (c != USER_TYPE_PROVIDER) {
                    addressCont.clear();
                    locationPickCont.clear();
                    providerLat = null;
                    providerLng = null;
                  }
                  setState(() {});
              ...
              if (selectedUserTypeValue == USER_TYPE_PROVIDER) ...[
                ...
                if (appConfigurationStore.isNearbyProviderEnable) ...[
                  12.height,
                  AppTextField(
                    ...
                    errorThisFieldRequired: languages.hintRequired,
                    validator: (value) {
                      if (value.validate().trim().isEmpty) {
                        return languages.hintRequired;
                      }
                      return null;
                    },
                    onTap: () async {
                      final result = await showAddressAutocompleteBottomSheet(
                        context,
                        initialAddress: addressCont.text.isNotEmpty
                            ? addressCont.text
                            : null,
                      );
                      if (result != null && mounted) {
                        setState(() {
                          ...
                          providerLat = result['latitude'] as double?;
                          providerLng = result['longitude'] as double?;
                        });
                      }
                    },
        ...
        AppTextField(
          ...
          validator: (val) {
            ...
            if (!regex.hasMatch(val)) {
              return languages.passwordValidationMsg;
            }
            return null;
          },
          onFieldSubmitted: (s) {
  ...
  void saveUser() async {
    if (formKey.currentState!.validate()) {
      ...
      if (selectedUserTypeValue == USER_TYPE_PROVIDER &&
          appConfigurationStore.isNearbyProviderEnable) {
        if (addressCont.text.trim().isEmpty) {
          return toast(languages.hintRequired);
        }
        if (providerLat == null || providerLng == null) {
          return toast(languages.lblPlsSelectAddress);
        }
      }
      ...
      if (selectedUserTypeValue == USER_TYPE_PROVIDER) {
        ...
        request[UserKeys.address] = addressCont.text.trim();
        request['latitude'] = providerLat;
        request['longitude'] = providerLng;
      }
      if (selectedUserTypeValue == USER_TYPE_HANDYMAN) {
      ...
      if (selectedUserTypeValue == USER_TYPE_PROVIDER) {
        appStore.setLoading(true);
        getDocTypeList(DocumentType.provider_document.name).then((docResponse) {
          appStore.setLoading(false);
          final hasDocs = docResponse.documents?.isNotEmpty ?? false;
          if (hasDocs) {
            UploadDocumentsScreen(formRequest: request).launch(context,
                pageRouteAnimation: PageRouteAnimation.SlideBottomTop);
          } else {
            // No documents configured by admin — register directly
            appStore.setLoading(true);
            registerUser(request).then((userRegisterData) {
              appStore.setLoading(false);
          ...
          appStore.setLoading(false);
          log('Doc list fetch failed, falling back to document screen: $e');
          // Fallback: show document screen even if API call fails
          UploadDocumentsScreen(formRequest: request).launch(context,
              pageRouteAnimation: PageRouteAnimation.SlideBottomTop);
        });
      } else {
