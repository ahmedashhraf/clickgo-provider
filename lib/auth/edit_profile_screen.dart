...
import '../components/address_autocomplete_bottom_sheet.dart';
...
class EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> init() async {
    ...
    _setPhoneFromStoredContact(appStore.userContactNumber);
    countryId = appStore.countryId;
    stateId = appStore.stateId;
    cityId = appStore.cityId;
    addressCont.text = appStore.address;
    providerLat = appStore.latitude;
    providerLng = appStore.longitude;
  ...
  Future<void> userDetailAPI() async {
    await getUserDetail(appStore.userId).then((value) async {
      ...
      providerLat = value.data!.latitude;
      providerLng = value.data!.longitude;

      _setPhoneFromStoredContact(value.data!.contactNumber.validate());
  ...
  void _setPhoneFromStoredContact(String contactNumber) {
    final parsedPhone = _phoneNumberService.parseStoredContact(contactNumber);

    if (parsedPhone.country != null) {
      selectedCountryPicker = parsedPhone.country!;
    }
    mobileCont.text = parsedPhone.localNumber;
  ...
  Future<void> update() async {
    ...
    multiPartRequest.fields[UserKeys.contactNumber] = _phoneNumberService.buildE164(
    ...
    if (providerLat != null) multiPartRequest.fields[AddAddressKey.latitude] = providerLat.toString();
    if (providerLng != null) multiPartRequest.fields[AddAddressKey.longitude] = providerLng.toString();
  ...
  Future<void> getZoneListApi() async {
    ...
    // Load all zones by paginating through all pages
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      AppTextField(
                        ...
                        focus: addressFocus,
                        minLines: 3,
                        isValidationRequired: appConfigurationStore.isNearbyProviderEnable,
                        readOnly: appConfigurationStore.isNearbyProviderEnable,
                        onTap: appConfigurationStore.isNearbyProviderEnable
                            ? () async {
                                final result = await showAddressAutocompleteBottomSheet(context, initialAddress: addressCont.text);
                                if (result != null) {
                                  addressCont.text = result['address'];
                                  providerLat = result['latitude'];
                                  providerLng = result['longitude'];
                                  setState(() {});
                                }
                              }
                            : null,
