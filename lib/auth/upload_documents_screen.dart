...
//get Document list
  ...
  void getMultipleFile({int? updateId, Function(String)? setImage}) async {
    filePickerResult = await FilePicker.pickFiles(allowMultiple: false, type: FileType.custom, allowedExtensions: ['jpg', 'png', 'jpeg', 'pdf']);
  ...
  Future<void> registerFun() async {
    ...
    if (uploadDocResp.isNotEmpty) {
      if (uploadDocResp.where((element) => (element.isRequired == 1 && (element.filePath?.isEmpty ?? true))).isNotEmpty) {
