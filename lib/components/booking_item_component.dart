...
class BookingItemComponent extends StatefulWidget {
  ...
  final Function(int bookingId, String updatedStatus)? onBookingStatusUpdated;
...
class BookingItemComponentState extends State<BookingItemComponent> {
  ...
  Future<void> updateBooking(BookingData booking, String updatedStatus, int index) async {
    ...
    await bookingUpdate(request).then((res) async {
      widget.bookingData.status = updatedStatus;
      widget.onBookingStatusUpdated?.call(widget.bookingData.id.validate(), updatedStatus);
      setState(() {});
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          if (widget.bookingData.shopInfo != null && widget.bookingData.shopInfo!.address.validate().isNotEmpty)
                            Container(
                        ...
                        await showConfirmDialogCustom(
                          ...
                          onAccept: (_) async {
                            ...
                            bookingUpdate(request).then((res) async {
                              ...
                              widget.onBookingStatusUpdated?.call(widget.bookingData.id.validate(), BookingStatusKeys.accept);
                              setState(() {});
                              LiveStream().emit(LIVESTREAM_UPDATE_BOOKINGS);
                              toast(res.message.validate());
                            }).catchError((e) {
