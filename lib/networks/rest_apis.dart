...
Future<void> saveUserData(UserData data) async {
  if (data.status == 1) {
    ...
    await appStore.setLatitude(data.latitude);
    await appStore.setLongitude(data.longitude);
...
Future<List<UserData>> getAllHandyman({
  ...
  try {
    UserListResponse res = UserListResponse.fromJson(
      await handleResponse(
        await buildHttpResponse(
          'user-list?user_type=handyman&provider_id=${appStore.userId}&status=1&per_page=$PER_PAGE_ITEM&page=$page',
          method: HttpMethodType.GET,
        ),
      ),
    );
...
Future<CommonResponseModel> updateServiceStatus(Map request) async {
  return CommonResponseModel.fromJson(
    await handleResponse(
      await buildHttpResponse(
...
Future<String> downloadSubscriptionInvoice({required int subscriptionId}) async {
  if (subscriptionId <= 0) throw errorSomethingWentWrong;

  MultipartRequest request = await getMultiPartRequest('download-subscription-invoice');
  request.fields['subscription_id'] = subscriptionId.toString();
  request.headers.addAll(buildHeaderTokensForMultipart());
  ...
  apiPrint(
    ...
    request: jsonEncode(request.fields),
    hasRequest: true,
  ...
  final String filePath = '${cacheDirectory.path}${Platform.pathSeparator}subscription_invoice_$subscriptionId.pdf';
...
Future<List<GooglePlacesModel>> getSuggestion(String input) async {
  ...
  String request = '$baseURL?input=${Uri.encodeComponent(input)}&key=${GOOGLE_PLACE_SEARCH_API}&sessiontoken=${appStore.token}';
...
/// Fetches place details (address, lat, lng) from Google Place Details API using [placeId].
Future<Map<String, dynamic>?> getPlaceDetails(String placeId) async {
  String baseURL = 'https://maps.googleapis.com/maps/api/place/details/json';
  String request = '$baseURL?place_id=${Uri.encodeComponent(placeId)}&fields=geometry,formatted_address&key=${GOOGLE_PLACE_SEARCH_API}';
...
Future<ShopDetailResponse> getShopDetail(int shopId) async {
  final res = await handleResponse(await buildHttpResponse('shop-detail/$shopId?provider_id=${appStore.userId}', method: HttpMethodType.GET));
...
Future<BaseResponseModel> saveShopHours(int shopId, Map<String, dynamic> request) async {
  return BaseResponseModel.fromJson(
    await handleResponse(
      await buildHttpResponse(
        'shop-hours-store/$shopId',
        request: request,
        method: HttpMethodType.POST,
      ),
    ),
...
Future<List<ShopDayModel>> getShopHoursList(int shopId) async {
  final res = await handleResponse(
    await buildHttpResponse('shop-hours-list/$shopId', method: HttpMethodType.GET),
  );
  List list = [];
  if (res is Map && res.containsKey('data')) {
    list = res['data'] is List ? res['data'] as List : [];
  } else if (res is List) {
    list = res;
  }
  return list
      .map((e) => ShopDayModel.fromJson(e is Map<String, dynamic> ? e : <String, dynamic>{}))
      .toList();
}
